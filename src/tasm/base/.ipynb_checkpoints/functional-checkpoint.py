...
def dice_coefficient(y_true, y_pred, beta=1.0, class_weights=1., smooth=1e-5, threshold=None):
    ...
    tp =K.sum(K.cast(y_true*y_pred, 'float'), axis=axes)
    fp =K.sum(K.cast((1-y_true)*y_pred, 'float'), axis=axes)
    fn =K.sum(K.cast(y_true*(1-y_pred), 'float'), axis=axes)
    # tf.print(tp,fp,fn)
    
    

    score = (((1.0 + beta) * tp) + smooth) / (((1.0 + beta) * tp) + (beta ** 2.0) * fn + fp + smooth)
    # tf.print("Score, wo avg: " + str(score))
    score = average(score, class_weights)
    # tf.print("Score: " + str(score))
